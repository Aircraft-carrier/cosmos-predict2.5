...
class _LossRecord:
    ...
    def get_stat(self) -> Tuple[float, float]:
        if self.iter_count > 0:
            ...
            avg_video_loss = self.video_loss / self.iter_count
            avg_action_loss = self.action_loss / self.iter_count
            dist.all_reduce(avg_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(avg_edm_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(avg_video_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(avg_action_loss, op=dist.ReduceOp.AVG)
            avg_loss = avg_loss.item()
            avg_edm_loss = avg_edm_loss.item()
            avg_video_loss = avg_video_loss.item()
            avg_action_loss = avg_action_loss.item()
        else:
            avg_loss = 0
            avg_edm_loss = 0
            avg_video_loss = 0
            avg_action_loss = 0
        self.reset()
        return avg_loss, avg_edm_loss, avg_video_loss, avg_action_loss


class Wandb4ActionCallback(Callback):
    """
    ...
    def on_training_step_end(
        ...
        if not skip_update_due_to_unstable_loss:
            if model.is_image_batch(data_batch):
                ...
                self.train_video_log.video_loss += output_batch["video_loss"].detach().float()
                self.train_video_log.action_loss += output_batch["action_loss"].detach().float()
            ...
            self.final_loss_log.video_loss += output_batch["video_loss"].detach().float()
            self.final_loss_log.action_loss += output_batch["action_loss"].detach().float()
        else:
            if model.is_image_batch(data_batch):
        ...
        if iteration % (self.config.trainer.logging_iter * self.logging_iter_multipler) == 0:
            ...
            avg_image_loss, avg_image_edm_loss, avg_image_video_loss, avg_image_action_loss = self.train_image_log.get_stat()
            avg_video_loss, avg_video_edm_loss, avg_video_video_loss, avg_video_action_loss = self.train_video_log.get_stat()
            avg_final_loss, avg_final_edm_loss, avg_final_video_loss, avg_final_action_loss = self.final_loss_log.get_stat()
            ...
            if distributed.is_rank0():
                ...
                info.update({
                    ...
                    # f"train{self.wandb_extra_tag}/image_video_loss": avg_image_video_loss,
                    # f"train{self.wandb_extra_tag}/image_action_loss": avg_image_action_loss,

                    # 视频批次指标
                    # f"train{self.wandb_extra_tag}/video_loss": avg_video_loss,
                    # f"train{self.wandb_extra_tag}/video_edm_loss": avg_video_edm_loss,
                    # f"train{self.wandb_extra_tag}/video_video_loss": avg_video_video_loss,
                    # f"train{self.wandb_extra_tag}/video_action_loss": avg_video_action_loss,

                    # 整体平均（所有批次）
                    f"train{self.wandb_extra_tag}/loss": avg_final_loss,
                    f"train{self.wandb_extra_tag}/edm_loss": avg_final_edm_loss,
                    f"train{self.wandb_extra_tag}/video_loss_total": avg_final_video_loss,
                    f"train{self.wandb_extra_tag}/action_loss_total": avg_final_action_loss,
