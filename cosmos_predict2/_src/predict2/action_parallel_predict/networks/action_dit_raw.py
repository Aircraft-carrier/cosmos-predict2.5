...
- [ ] Step 5: Use different timestep sampling strategies for action and video generation. 
"""
...
class ActionRopePositionEmb(nn.Module):
    ...
    def generate_embeddings(
        ...
        # Repeat each frequency twice to match full head_dim (for cos/sin)
        # Equivalent to: [θ0, θ0, θ1, θ1, ..., θ_{k-1}, θ_{k-1}]
        emb = torch.cat([half_emb, half_emb], dim=-1)                  # [S, head_dim]
        # Reshape to match video RoPE format: [S, 1, 1, head_dim]
...
class ActionAwareFinalLayer(nn.Module):
    """
    ...
    def __init__(
        ...
        if use_adaln_lora:
            self.adaln_modulation = nn.Sequential(
                nn.SiLU(),
                nn.Linear(2 * hidden_size, adaln_lora_dim, bias=False),
                nn.Linear(adaln_lora_dim, self.n_adaln_chunks * hidden_size, bias=False),
            )
        else:
            self.adaln_modulation = nn.Sequential(
                nn.SiLU(), nn.Linear(2 * hidden_size, self.n_adaln_chunks * hidden_size, bias=False)
            )
    ...
    def forward(
        ...
        with amp.autocast("cuda", enabled=self.use_wan_fp32_strategy, dtype=torch.float32):
            if self.use_adaln_lora:
                ...
                print("shift_self_attn_B_T_D", shift_self_attn_B_T_D[0][0][:5])
                print("scale_self_attn_B_T_D", scale_self_attn_B_T_D[0][0][:5])
                print("gate_self_attn_B_T_D", gate_self_attn_B_T_D[0][0][:5])
                shift_cross_attn_B_T_D, scale_cross_attn_B_T_D, gate_cross_attn_B_T_D = (
                    self.adaln_modulation_cross_attn(emb_B_T_D) + adaln_lora_B_T_3D
                ).chunk(3, dim=-1)
                print("shift_cross_attn_B_T_D", shift_cross_attn_B_T_D[0][0][:5])
                print("scale_cross_attn_B_T_D", scale_cross_attn_B_T_D[0][0][:5])
                print("gate_cross_attn_B_T_D", gate_cross_attn_B_T_D[0][0][:5])    
                shift_mlp_B_T_D, scale_mlp_B_T_D, gate_mlp_B_T_D = (
                    self.adaln_modulation_mlp(emb_B_T_D) + adaln_lora_B_T_3D
                ).chunk(3, dim=-1)   
                print("shift_mlp_B_T_D", shift_mlp_B_T_D[0][0][:5])
                print("scale_mlp_B_T_D", scale_mlp_B_T_D[0][0][:5])
                print("gate_mlp_B_T_D", gate_mlp_B_T_D[0][0][:5])               
            else:
...
class ActionGenerationRawMinimalV1LVGDiT(MiniTrainDIT):  
    def __init__(self, *args, timestep_scale: float = 1.0, **kwargs):
        ...
        super().__init__(*args, **kwargs) 
        # self.t_embedder.state_dict() -> {
        #     "1.linear_1.weight": Tensor of shape [model_channels, model_channels],
        #     "1.linear_1.bias":   Tensor of shape [model_channels],if no AdaLN LoRA
        #     "1.linear_2.weight": Tensor of shape [model_channels, model_channels]
        # }
        ...
        # self.final_layer = ActionAwareFinalLayer(
        ...
        # 1226 modified
        # self.t_embedder.state_dict() ->{  
        #   'mlp.weight':           Tensor of shape [model_channels, 2 * model_channels], 
        #   'mlp.bias':             Tensor of shape [model_channels], 
        #   'proj.linear_1.weight': Tensor of shape [model_channels, model_channels], 
        #   'proj.linear_2.weight': Tensor of shape [3 * model_channels, model_channels]
        # }
    ...
    def forward(
        ...
        **kwargs,
        ...
        with amp.autocast("cuda", enabled=self.use_wan_fp32_strategy, dtype=torch.float32):
            ...
            print("t_embedding_B_T_D: ", t_embedding_B_T_D[0][0][:5])
            print("adaln_lora_B_T_3D: ", adaln_lora_B_T_3D[0][0][:5])
        ...
        action = self.action_head(action_B_T_D)        # [B, 17, 14]
...
def main():
    ...
    video_net = ActionGenerationRawMinimalV1LVGDiT(**net_config)
