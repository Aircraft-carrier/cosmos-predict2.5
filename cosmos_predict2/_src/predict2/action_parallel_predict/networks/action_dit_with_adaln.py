...
- [x] Step 5: Use different timestep sampling strategies for action and video generation. (2025-12-20)
"""
...
class ActionRopePositionEmb(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Split dimensions equally among T, H, W
        dim_h = head_dim // 6 * 2     # 341 * 2 = 682          
        dim_t = head_dim - 2 * dim_h  # 684 = 342 * 2

        self._dim_h = dim_h # 42
        self._dim_t = dim_t # 44

        # NTK extrapolation factors
        self.ntk_factor = extrapolation_ratio ** (dim_t / (dim_t - 2)) if dim_t > 2 else 1.0
        
        # Shared sequence buffer [0, 1, ..., max_action_len - 1]
        ...
        # Frequency ranges (scaled by their own dim)
        self.register_buffer(
            "dim_spatial_range",     # shared for H and W
            torch.arange(0, dim_h, 2)[: (dim_h // 2)].float() / dim_h,
            persistent=True,
        )                            # 21
        self.register_buffer(
            "dim_temporal_range",    # T
    ...
    def generate_embeddings(
        ...
        ntk_factor = ntk_factor or self.ntk_factor
        theta = self.base * ntk_factor
        # Frequencies
        spatial_freqs  = 1.0 / (theta ** self.dim_spatial_range)   #     # [d_h//2] (used for both H and W)
        temporal_freqs = 1.0 / (theta ** self.dim_temporal_range)  #     # [d_t//2]
        
        # Use the SAME sequence indices for T, H, W
        m = self.seq[:num_action_tokens]  # [S]
        half_emb_h = torch.outer(m, spatial_freqs)    # [S, d_h//2]
        half_emb_t = torch.outer(m, temporal_freqs)   # [S, d_t//2]
        
        emb = torch.cat([half_emb_t, half_emb_h, half_emb_h] * 2, dim=-1) # [S, head_dim]
        return rearrange(emb, "s d -> s 1 1 d").float()

    @property
    def seq_dim(self) -> int:
        return 0   
      
class ActionAwareBlock_2adaln(Block):
    """
    ...
    def forward(
        ...
        action_timesteps_B_T_D: Optional[torch.Tensor] = None,
        action_adaln_lora_B_T_3D: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:    
        ...
        with amp.autocast("cuda", enabled=self.use_wan_fp32_strategy, dtype=torch.float32):
            if self.use_adaln_lora:
                # video part
                self_attn_video = (self.adaln_modulation_self_attn(emb_B_T_D) + adaln_lora_B_T_3D).chunk(3, dim=-1)  # (shift, scale, gate)
                cross_attn_video = (self.adaln_modulation_cross_attn(emb_B_T_D) + adaln_lora_B_T_3D).chunk(3, dim=-1)
                mlp_video = (self.adaln_modulation_mlp(emb_B_T_D) + adaln_lora_B_T_3D).chunk(3, dim=-1)
                # action part
                self_attn_action = (self.adaln_modulation_self_attn(action_timesteps_B_T_D) + action_adaln_lora_B_T_3D).chunk(3, dim=-1)
                cross_attn_action = (self.adaln_modulation_cross_attn(action_timesteps_B_T_D) + action_adaln_lora_B_T_3D).chunk(3, dim=-1)
                mlp_action = (self.adaln_modulation_mlp(action_timesteps_B_T_D) + action_adaln_lora_B_T_3D).chunk(3, dim=-1)  
                              
            else:
                self_attn_video = self.adaln_modulation_self_attn(emb_B_T_D).chunk(3, dim=-1)
                cross_attn_video = (self.adaln_modulation_cross_attn(emb_B_T_D).chunk(3, dim=-1))
                mlp_video = self.adaln_modulation_mlp(emb_B_T_D).chunk(3, dim=-1)
        ...
        def _make_unified_modulation(
            shift1_B1D, scale1_B1D, gate1_B1D,   # e.g., for video
            shift2_B1D, scale2_B1D, gate2_B1D,   # e.g., for action
        ):
            # Expand each to its token count (no memory copy, just view)
            shift1_exp = shift1_B1D.expand(-1, L, -1)   # [B, N1, D]
            scale1_exp = scale1_B1D.expand(-1, L, -1)
            gate1_exp  = gate1_B1D.expand(-1, L, -1)

            shift2_exp = shift2_B1D.expand(-1, S, -1)   # [B, N2, D]
            scale2_exp = scale2_B1D.expand(-1, S, -1)
            gate2_exp  = gate2_B1D.expand(-1,  S, -1)

            # Concatenate along token dimension
            shift_unified = torch.cat([shift1_exp, shift2_exp], dim=1).to(dtype)  # [B, N1+N2, D]
            scale_unified = torch.cat([scale1_exp, scale2_exp], dim=1).to(dtype)
            gate_unified  = torch.cat([gate1_exp,   gate2_exp], dim=1).to(dtype)

            return shift_unified, scale_unified, gate_unified            
        ...
        shift_self_attn, scale_self_attn, gate_self_attn = _make_unified_modulation(
            *self_attn_video,
            *self_attn_action
        )
        ...
        unified_B_L_D = unified_B_L_D + result_B_L_D * gate_self_attn  # [B, 1616, 2048]
        ...
        shift_cross_attn, scale_cross_attn, gate_cross_attn = _make_unified_modulation(
            *cross_attn_video,
            *cross_attn_action
        )
        ...
        unified_B_L_D = unified_B_L_D + result_B_L_D * gate_cross_attn # [B, 1616, 2048]
        
        shift_mlp, scale_mlp, gate_mlp = _make_unified_modulation(
            *mlp_video,
            *mlp_action
        )
        ...
        unified_B_L_D = unified_B_L_D + result_B_L_D * gate_mlp
...
class DualTimestepEncoder(nn.Module):
    def __init__(self, model_channels: int = 512, use_adaln_lora: bool = False):
        ...
        self.video_proj = TimestepEmbedding(model_channels, model_channels, use_adaln_lora=use_adaln_lora)
        self.action_proj = TimestepEmbedding(model_channels, model_channels, use_adaln_lora=use_adaln_lora)
        
    def forward(self, t1, t2):
        temb1 = self.sinusoidal_pos_emb(t1)
        temb2 = self.sinusoidal_pos_emb(t2)
        emb1_B_T_D, adaln_lora1_B_T_3D = self.video_proj(temb1)
        emb2_B_T_D, adaln_lora2_B_T_3D = self.video_proj(temb2)
        return emb1_B_T_D, adaln_lora1_B_T_3D, emb2_B_T_D, adaln_lora2_B_T_3D
    
    def init_weights(self) -> None:
        """Initialize MLP weights so that it initially acts as an identity mapping for the first half of inputs."""
        self.video_proj.init_weights()
        self.action_proj.init_weights()    

class ActionGeneration2MinimalV1LVGDiT(MiniTrainDIT):  
    def __init__(self, *args, timestep_scale: float = 1.0, **kwargs):
        ...
        self.action_t_embedding_norm = te.pytorch.RMSNorm(self.model_channels, eps=1e-6)
        
        
        self.blocks = nn.ModuleList(           
            [
                ActionAwareBlock_2adaln(
    ...
    def init_weights(self):
        ...
        if hasattr(self, 'action_embedder'):
            ...
            self.t_embedder[1].init_weights()  # 在 full share 那里还有用到，注意看一看！
    ...
    def forward(
        ...
        **kwargs,
        ...
        with amp.autocast("cuda", enabled=self.use_wan_fp32_strategy, dtype=torch.float32):
            ...
            t_embedding_B_T_D, adaln_lora_B_T_3D = self.t_embedder(timesteps_B_T) # [B, 1, 2048] [B, 1, 6144]      
        ...
        for i, block in enumerate(self.blocks):
            x_B_T_H_W_D, action_B_T_D = block(
                ...
                action_timesteps_B_T_D=action_t_embedding_B_T_D,
                action_adaln_lora_B_T_3D=action_adaln_lora_B_T_3D,
            )
        ...
        action = self.action_head(action_B_T_D, action_t_embedding_B_T_D, adaln_lora_B_T_2D=action_adaln_lora_B_T_3D)  # [B, 17, 14]
...
def main():
    ...
    video_net = ActionGeneration2MinimalV1LVGDiT(**net_config)
