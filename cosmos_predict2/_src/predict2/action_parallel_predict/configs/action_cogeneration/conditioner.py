...
@dataclass(frozen=True)
class ActionGenerationCondition(Video2WorldCondition):
    use_action_congengen: bool = True 
    
class ActionGenerationConditioner(Video2WorldConditioner):
    def forward(
        self,
        batch: Dict,
        override_dropout_rate: Optional[Dict[str, float]] = None,
        ...
        output = super()._forward(batch, override_dropout_rate)
        return ActionGenerationCondition(**output)
    ...
    def get_condition_uncondition(
        ...
        for emb_name, embedder in self.embedders.items():
            ...
            dropout_rates[emb_name] = 1.0 if embedder.dropout_rate > 1e-4 and "text" not in emb_name else 0.0
        # LINK: cosmos_predict2/_src/predict2/configs/video2world/defaults/conditioner.py:231    
        # The conditioner's forward applies dropout to the specified inputs and produces a condition instance.
...
ActionGenerationConditionerConfig: LazyDict = L(ActionGenerationConditioner)(
    **_SHARED_CONFIG,
)
...
def register_conditioner():
    ...
    cs.store(
        group="conditioner",
        package="model.config.conditioner",
        name="action_generation_conditioner",
        node=ActionGenerationConditionerConfig,
    )
